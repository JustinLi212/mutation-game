...
@onready var gun_progress_bar: ProgressBar = $GunProgressBars/RedProgressBar
@onready var red_time_left: RichTextLabel = $GunProgressBars/RedProgressBar/RedTimeLeft
...
func _ready() -> void:
	...
	for sprite: AnimatedSprite2D in grid_labels.get_children():
		...
		sprite.play()
	for progress_bar: ProgressBar in gun_progress_bars.get_children():
...
func _on_gun_timer_changed(time_left: float, wait_time: float) -> void:
	red_time_left.text = "%.2f" % time_left
